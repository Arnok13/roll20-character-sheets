    ...
    on("sheet:opened", function() {
        getAttrs(["sheetVersion", "atkcible"], function(values) {
            ...
            update["text_arcane"] = getTranslationByKey("arcane");
            ...
            update["text_crochetage"] = getTranslationByKey("crochetage");
            ...
            update["text_escalade"] = getTranslationByKey("escalade");
            ...
            update["text_natation"] = getTranslationByKey("natation");
            ...
            update["text_portee"] = getTranslationByKey("portee");
            ...
            if (vers < 3.0) {
                ...
                getAttrs(["pnjtype", "extra", "pnj"], function(values) {
                    ...
                    var pnjval = ((parseInt(values.extra) || 0) * 3) + (parseInt(values.pnj) || 0);
                    if ((pnjtyp == 0) && (pnjval > 0)) {
                        update["pnjtype"] = pnjval;
                    }
            ...
            if (vers < 3.2) {
                if ((parseInt(values.atkcible) || 0) == 1) {
                    ...
                    update["cibleresistance"] = "?{" + getTranslationByKey("resistance") + "|4}";
                    update["ciblearcane"] = "?{" + getTranslationByKey("difficulte") + "|4}";
                    update["atknom"] = "-";
                }
                update["sheetVersion"] = 3.2;
            }
            setAttrs(update);
        });
    });
    ...
    on("change:confBlessures", function(eventinfo) {
        ...
        setAttrs({
            bless1: parseInt(+(conf > 0)) || 0,
            bless2: parseInt(+(conf > 1)) || 0,
            bless3: parseInt(+(conf > 2)) || 0
        });
    });
    on("change:blessures", function(eventinfo) {
        var malusblessures = 0;
        if (eventinfo.newValue > 3) {
            malusblessures = 3;
        } else {
    ...
    on("change:force_de", function(eventinfo) {
        setAttrs({
            encombmax: parseInt(eventinfo.newValue) * 2.5
        });
    });
    ...
    on("change:atkcible", function(eventinfo) {
        ...
        if (eventinfo.newValue == 1) {
            ...
            cibleparade = "?{" + getTranslationByKey("parade") + "|4}";
